 ...
 * @remark 实现观察者模式进行状态更新，支持会话管理和消息统计
 */
 ...
 * 会话状态变化的回调类型
 * @callback Listener
 * @param {Conversation[]} conversations - 更新后的会话数组
 */
...
class ConversationStore {
   ...
   * 创建新对话会话
   * @async
   * @param {string} [title] - 会话标题，如果不提供则使用默认标题
   * @returns {Promise<number>} 新创建会话的 ID
   * @since 2025-12-24
   */
   ...
   * 更新会话信息
   * @async
   * @param {number} id - 要更新的会话 ID
   * @param {Partial<Conversation>} updates - 要更新的字段
   * @since 2025-12-24
   */
   ...
   * @param {number} delta - 消息数量的变化量
   * @since 2025-12-24
   */
  async updateMessageCount(id: number, delta: number) {
    const conversation = await db.conversations.get(id)
    if (!conversation) return
  ...
  /**
   * 获取当前会话
   * @returns {Conversation | null} 当前会话对象
   * @since 2025-12-24
   */
